...
namespace Thinhnv.UnityTools.AssetChildConverter
{
    ...
    public partial class AssetChildConverterWindow
    {
        ...
        private void DrawChildrenSection()
        {
            ...
            bool foreign = AssetDatabase.IsForeignAsset(parentAsset);
            if (foreign)
            {
                EditorGUILayout.HelpBox(
                    $"'{parentAsset.name}' is an imported asset. Its sub-assets are regenerated by its importer, " +
                    "so they cannot be removed from it — they can only be copied out as standalone assets.",
                    MessageType.Warning);
            }
        ...
        private void DrawUnpackControls(bool parentIsForeign)
        {
            ...
            using (new EditorGUI.DisabledScope(parentIsForeign))
            {
                bool remove = parentIsForeign ? false : removeAfterUnpack;
                remove = EditorGUILayout.ToggleLeft(
                    "Remove from parent after extracting (leave off to copy out)", remove);
                if (!parentIsForeign) removeAfterUnpack = remove;
            }
            ...
            using (new EditorGUI.DisabledScope(blocked))
            {
                ...
                bool remove = removeAfterUnpack && !parentIsForeign;
                if (clicked && ConfirmUnpack(chosen.Count, remove))
                {
                    lastLog.Clear();
                    Unpack(chosen, parentAsset, fixReferences, unpackFolder, remove);
                    InvalidateScan();
                }
            }
        }
